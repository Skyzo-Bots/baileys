...
// modifikasi di sini untuk menangani rate-limit tanpa langsung crash total
const assertNodeErrorFree = (node) => {
    ...
    if (errNode) {
        ...
        // jika error adalah rate-limit (429), kita berikan log peringatan saja
        if (errorCode === 429) {
            console.warn(`[whatsapp rate-limit] server busy: ${errorText}`)
        }
        
        throw new boom_1.Boom(errorText, { data: errorCode })
    }
}
